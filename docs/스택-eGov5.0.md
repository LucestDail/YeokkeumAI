...
| Server Security | **Spring Security 6.5.5** | Apache 2.0 | ✅ Security FilterChain 이관 완료(`SecurityConfig`+`TokenAuthFilter`, RBAC·secure-by-default) |
...
| Rest API 문서 | springdoc 2.8.13 | Apache 2.0 | ✅ Swagger UI 적용(`OpenApiConfig`, `/swagger-ui/index.html`, Bearer 스킴) |
...
| Excel/문서 | POI 5.4.0, jXLS 3.0.0 | Apache 2.0 | ✅ DOCX/XLSX 텍스트 추출(POI, `DocText`) · 리포팅(jXLS)은 🔜 |
...
## 요약
- **이번 적용**: Spring Boot 3.5.6 · Spring AI 1.0.1(AI layer) · JPA/Hibernate · JUnit5 · 비root 컨테이너 · Spring Security 이관 · springdoc(Swagger) · KRDS Vue3 프론트 · POI(DOCX/XLSX).
- **다음(정렬 잔여)**: (필요시) MyBatis · Batch · Quartz · EHCache · jasypt · jXLS 리포팅 · 관측스택(Prometheus/Grafana).
