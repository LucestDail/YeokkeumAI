...
## [R3] 외부 모델로 나가는 개인정보 — ✅ 2026-09-16 조치

**결정: 보내기 전 마스킹**(사용자 판단). 근거 = 공공 RFP 문서(계약서·신청서)에 개인정보가 흔하고,
이 제품이 가장 터질 수 있는 지점이기 때문이다.

🔴 **조치 전 상태**: `PiiMasker` 는 예전부터 있었는데 **`AuditService` 한 곳만** 쓰고 있었다 —
**로그에는 안 남기면서 외부 모델에는 원문을 그대로 보내고** 있었다.

- 나가는 길은 **둘**이다 — 채팅(`/chat/completions`)과 **임베딩**(`/embeddings`, RAG 청크 원문).
  **한 곳만 막으면 반쪽**이라 `OutboundPii` 를 양쪽에 걸었다.
- 설정: `yeokkeum.llm.mask-pii` · `yeokkeum.embedding.mask-pii` (둘 다 **기본 켜짐**, 환경변수로 끌 수 있음)
- ⚠️ **끄면 문서 원문이 나간다.** "계약서의 연락처가 뭐야" 같은 질문에 답이 필요하면 끄되,
  **무엇이 나가는지 알고** 끄는 것이어야 한다.
- ⚠️ **몇 건 가렸는지 로그로 낸다**(`[pii]`) — 0이면 "설정은 켰는데 실제로는 안 돌았다" 를
  결과가 스스로 알려 준다. **가린 내용 자체는 안 남긴다**(남기면 마스킹한 의미가 없다).
- 가리는 대상은 **모양이 분명한 것**뿐(주민번호·전화·카드·이메일). 이름·주소처럼 경계가 모호한 것은
  넣지 않았다 — 모호한 것을 자에 넣으면 자가 거짓말한다.

**가드**: `OutboundPiiWiringRuleTest` 가 외부 송신 파일을 **거꾸로 계정**한다(전부
`마스킹함`/`면제됨(이유)` 중 하나여야 함). 새 게이트웨이를 마스킹 없이 추가하면 **파일명을 찍어** 실패한다.

⚠️ **남은 것**: 응답(LLM → 우리)의 PII 는 손대지 않았다. 이미 외부로 나간 뒤라 가려도 보호가 안 되고,
화면·감사 로그 쪽은 `PiiMasker` 가 이미 맡고 있다.
